 ...
 */
@Mapper(componentModel = "spring", uses = {ElasticShippingClaimMapper.class})
public interface ElasticsearchStatusMapper extends EntityMapper<ElasticsearchStatusDTO, ElasticsearchStatus> {

    @Mapping(source = "elasticShippingClaim.id", target = "elasticShippingClaimId")
    ElasticsearchStatusDTO toDto(ElasticsearchStatus elasticsearchStatus);

    @Mapping(source = "elasticShippingClaimId", target = "elasticShippingClaim")
